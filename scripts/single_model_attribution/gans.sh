...
do
    ...
    do
        ...
        CUDA_VISIBLE_DEVICES=$DEV python3 sma_deepsad.py --avoid-caching --train-dir $train_dir --real-dir real --dataset $dataset --model $generator --checkpoint-path $checkpoint_path --num-train 10000 --lr 0.0005 --feat raw --net-name cifar10_LeNet --n-epochs $n_epochs --output-path $output_path
        # center crop DCTPAD
        CUDA_VISIBLE_DEVICES=$DEV python3 sma_deepsad.py --avoid-caching --train-dir $train_dir --real-dir real --dataset $dataset --model $generator --checkpoint-path $checkpoint_path --num-train 10000 --lr 0.0005 --feat dct --net-name cifar10_LeNet --n-epochs $n_epochs --output-path $output_path --downsampling center_crop
        # Fingerprint by Marra
        CUDA_VISIBLE_DEVICES=$DEV python3 sma_baselines.py --attr fingerprint --train-dir $train_dir --real-dir real --dataset $dataset --model $generator --checkpoint-path $checkpoint_path --output-path $output_path
    done
done

# Inversion-based methods. Running inversion multiple times is computationally too expensive
for generator in "dcgan" "wgangp" "lsgan" "ebgan"
do
    train_dir="${generator}_1"
    checkpoint_path=${checkpoint_paths[$generator]}
    
    CUDA_VISIBLE_DEVICES=$DEV python3 sma_baselines.py --attr l2_inversion --train-dir $train_dir --real-dir real --dataset celeba_0 --model $generator --checkpoint-path $checkpoint_path --output-path $output_path
    CUDA_VISIBLE_DEVICES=$DEV python3 sma_baselines.py --attr inception_inversion --train-dir $train_dir --real-dir real --dataset celeba_0 --model $generator --checkpoint-path $checkpoint_path --output-path $output_path
done
...
do
    ...
    CUDA_VISIBLE_DEVICES=$DEV python3 sma_baselines.py --attr l2_inversion --train-dir $train_dir --real-dir real --dataset lsun_0 --model $generator --checkpoint-path $checkpoint_path --output-path $output_path
    CUDA_VISIBLE_DEVICES=$DEV python3 sma_baselines.py --attr inception_inversion --train-dir $train_dir --real-dir real --dataset lsun_0 --model $generator --checkpoint-path $checkpoint_path --output-path $output_path
done
