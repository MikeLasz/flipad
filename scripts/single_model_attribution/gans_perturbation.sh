...
declare -A perturb_params
perturb_params["noise"]="0.05 0.1"
perturb_params["blur"]="1 3"
perturb_params["crop"]="60 55"
perturb_params["jpeg"]="90 80"
...
do
    ...
    do
        for perturb_type in "noise" "jpeg" "blur" "crop"
        do
            perturb_param=${perturb_params[$perturb_type]}
            for param in $perturb_param
            do
                train_dir="${generator}_1"
                checkpoint_path=${checkpoint_paths[$generator]} 
                # max downsamping FLIPAD
                CUDA_VISIBLE_DEVICES=$DEV python3 sma_deepsad.py --perturbation $perturb_type --perturbation-param $param --immunization --avoid-caching --train-dir $train_dir --real-dir real --dataset $dataset --model $generator --checkpoint-path $checkpoint_path --rec-lr 0.025 --rec-max-iter $rec_max_iter --rec-alpha 0.0005 --num-train $n_train --lr 0.0005 --n-epochs $n_epochs --output-path $output_path --downsampling max --net-name lenet_64channels --num_select_channels 64
                # bicubic downsampling RAWPAD
                CUDA_VISIBLE_DEVICES=$DEV python3 sma_deepsad.py --perturbation $perturb_type --perturbation-param $param --immunization --avoid-caching --train-dir $train_dir --real-dir real --dataset $dataset --model $generator --checkpoint-path $checkpoint_path --num-train $n_train --lr 0.0005 --feat raw --net-name cifar10_LeNet --n-epochs $n_epochs --output-path $output_path
                # center crop DCTPAD
                CUDA_VISIBLE_DEVICES=$DEV python3 sma_deepsad.py --perturbation $perturb_type --perturbation-param $param --immunization --avoid-caching --train-dir $train_dir --real-dir real --dataset $dataset --model $generator --checkpoint-path $checkpoint_path --num-train $n_train --lr 0.0005 --feat dct --net-name cifar10_LeNet --n-epochs $n_epochs --output-path $output_path --downsampling center_crop
            done
        done
    done
done
    ...
    do
        ...
        do
            ...
            do
                ...
                CUDA_VISIBLE_DEVICES=$DEV python3 sma_deepsad.py --perturbation $perturb_type --perturbation-param $param --immunization --avoid-caching --train-dir $train_dir --real-dir real --dataset $dataset --model $generator --checkpoint-path $checkpoint_path --rec-lr 0.00075 --rec-max-iter $rec_max_iter --rec-alpha 0.0005 --num-train $n_train --lr 0.0005 --n-epochs $n_epochs --output-path $output_path --downsampling max --net-name lenet_64channels --num_select_channels 64
