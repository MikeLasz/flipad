...
def rings_log_pdf(X, sigma=0.1, radia=np.array([1, 3])):
    ...
    if X.shape[1] > 2:
        ...
        result += np.sum(
            -0.5 * np.log(2 * np.pi * sigma**2) - 0.5 * (X[:, 2:] ** 2) / (sigma**2), 1
        )
...
class Spiral(ToyDataset):
    ...
    def __init__(
        ...
        eps=1,
        r_scale=1.5,
        starts=np.array([0.0, 2.0 / 3, 4.0 / 3]) * np.pi,
        length=np.pi,
    ):
    ...
    def sample(self, n):
        ...
        for si, s in enumerate(self.starts):
            ...
            data[si * batch_size : (si + 1) * batch_size] = self.sample_branch(
                batch_size, s
            )
        return data[:n, :]
    ...
    def _conditional_dpdf_one_dim(self, a, x, D):
        ...
        for si, s in enumerate(self.starts):
            ...
            m, s = self._branch_params_one(a, s)
            dpdf[si] = (
                np.exp(norm.logpdf(x, loc=m, scale=s).sum())
                * (-x[D] + m[D])
                / s[D] ** 2
            )
            dpdf[si] /= self.nstart
    ...
    def dpdf_one(self, x, *args, **kwargs):
        ...
        for d in range(self.D):
            dpdf[d] = quad(
                self._conditional_dpdf_one_dim, 0, 1, (x, d), *args, **kwargs
            )[0]
        return dpdf
...
class Banana(ToyDataset):
    ...
    def logpdf_multiple(self, x):
        ...
        logp = norm.logpdf(x[:, 0], 0, self.sigma) + norm.logpdf(
            x[:, 1], self.bananicity * (x[:, 0] ** 2 - self.sigma**2), 1
        )
        if self.D > 2:
...
class RealDataset(Dataset):
    ...
    def stream(self, n, add_noise=False):
        ...
        d = np.take(
            self.data, range(self.pointer, self.pointer + n), mode="wrap", axis=0
        )
        if self.nkde:
            p = np.take(
                self.kde_logp,
                range(self.pointer, self.pointer + n),
                mode="wrap",
                axis=0,
            )
        else:
    ...
    def mvn_score(self, x=None):
        ...
        if x is None:
            x = self.test_data
        return 0.5 * np.sum(
            np.linalg.solve(c, (self.test_data - m).T) ** 2, 0
        ) - np.trace(np.linalg.inv(c))
...
class WhiteWine(RealDataset):
    ...
    def __init__(self, *args, **kwargs):
        self.data = np.loadtxt(
            "/USERSPACE/DATASETS/wine+quality/winequality-white.csv",
            delimiter=";",
            skiprows=1,
        )[:, :-1]
        self.name = "WhiteWine"
...
class RedWine(RealDataset):
    ...
    def __init__(self, *args, **kwargs):
        self.data = np.loadtxt(
            "/USERSPACE/DATASETS/wine+quality/winequality-red.csv",
            delimiter=";",
            skiprows=1,
        )[:, :-1]
        self.name = "RedWine"
...
class Parkinsons(RealDataset):
    def __init__(self, cor=0.98, *args, **kwargs):
        self.data = np.loadtxt(
            "/USERSPACE/DATASETS/parkinsons/parkinsons.data",
            skiprows=1,
            usecols=range(4, 24),
            delimiter=",",
        )
        # np.loadtxt("data/parkinsons/parkinsons.data",
        #      delimiter=",", skiprows=1)[:, 3:]
        # drop status
        self.data = np.delete(self.data, 13, axis=1)
...
class Gas(RealDataset):
    def __init__(self, cor=0.98, *args, **kwargs):
        self.data = np.array(np.load("data/ethylene_CO.pickle", encoding="latin1"))[
            :, 3:
        ]
        self.name = "Gas"
        self.data = clean_data(self.data, cor=cor)
