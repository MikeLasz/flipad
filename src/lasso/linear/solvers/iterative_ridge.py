...
def iterative_ridge(
    ...
    """
    if tikhonov < 1e-5:
        warnings.warn("small regularization value %0.4e may lead to " "inprecise results." % tikhonov)
    ...
    for k in range(1, maxiter + 1):
        ...
        rhs_k = rhs.masked_fill(is_zero, 0.0)

        # solve ridge sub-problem
        if cg:
            # use conjugate gradient method
            def Adot(v):
                Av = torch.mm(v.masked_fill(is_zero, 0.0), A)
                Av.masked_fill_(is_zero, 0.0)
            ...
            A_k = A.masked_fill((is_zero.unsqueeze(1) | is_zero.unsqueeze(2)), 0.0)
