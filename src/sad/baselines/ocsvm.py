...
class OCSVM(object):
    ...
    def train(self, dataset: BaseADDataset, device: str = "cpu", n_jobs_dataloader: int = 0):
        ...
        X_val = np.concatenate(
            (X_test[perm][labels[perm] == 0][:n_val_normal], X_test[perm][labels[perm] == 1][:n_val_outlier])
        )
    ...
    def test(self, dataset: BaseADDataset, device: str = "cpu", n_jobs_dataloader: int = 0):
        ...
        if self.hybrid:
            ...
            logger.info("Test AUC linear model: {:.2f}%".format(100.0 * self.results["test_auc_linear"]))
