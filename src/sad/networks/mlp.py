...
class MLP(BaseNet):
    def __init__(self, x_dim, h_dims=[128, 64], rep_dim=32, bias=False, dropout=0.0):
        super().__init__()

        self.rep_dim = rep_dim

        neurons = [x_dim, *h_dims]
        layers = [Linear_BN_leakyReLU(neurons[i - 1], neurons[i], bias=bias, dropout=dropout) for i in range(1, len(neurons))]
...
class Linear_BN_leakyReLU(nn.Module):
    ...
    def __init__(self, in_features, out_features, bias=False, eps=1e-04, dropout=0.0):
    ...
    def forward(self, x):
        return self.dropout(F.leaky_relu(self.bn(self.linear(x))))
